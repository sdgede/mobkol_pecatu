...
class GlobalProvider extends ChangeNotifier {
  ...
  GlobalCollectionServices globalCollectionServices = setup<GlobalCollectionServices>();
  ...
  Future getLogin(BuildContext context, String username, String password) async {
    ...
    try {
      ...
      var result = res;
      if (_connectionMode == config.onlineMode) result = res == null ? null : json.decode(res);
      _dataLogin = result;
      ...
      if (result == null) {
        ...
        EasyLoading.dismiss();
        DialogUtils.instance.showError(context: context, text: result['responLogin']['pesan']);
      } else {
  ...
  Future syncMigrateData({BuildContext? context, var groupProduk, var rekCd, DateTime? tglAwal, DateTime? tglAkhir}) async {
  ...
  Future syncData(BuildContext context, var products) async {
    try {
      ...
      var effectedRowCF = await syncMigrateData(context: context, groupProduk: 'CONFIG', rekCd: 'CONFIG', tglAwal: last, tglAkhir: current);
      if (effectedRowCF != null) {
        updatedText += ' Config,';
        ditambahkan += effectedRowCF['row_add'] as int;
        diperbaharui += effectedRowCF['row_edit'] as int;
      }
      var effectedRowMN = await syncMigrateData(context: context, groupProduk: 'MASTER_NASABAH', rekCd: 'MASTER_NASABAH', tglAwal: last, tglAkhir: current);
      ...
      for (ProdukCollection product in products) {
        var effectedRow = await syncMigrateData(context: context, groupProduk: product.slug, rekCd: product.rekCd, tglAwal: last, tglAkhir: current);
        if (effectedRow != null) {
      ...
      await DialogUtils.instance.showInfo(
        ...
        isCancel: false,
        title: "Pemberitahuan!",
        text: "Data berhasil diperbaharui \n\n" + "Data ditambahkan : " + ditambahkan.toString() + "\n" + "Data diperbarui : " + diperbaharui.toString() + "\n\nData yang diperbaharui: $updatedText",
        clickOKText: "TUTUP",
      );
      return true;
    } catch (e) {
  ...
  dynamic getLoginPin(BuildContext context, String pin) async {
    ...
    try {
      ...
      if (result == null) {
        ...
        Navigator.of(context).pushNamedAndRemoveUntil(RouterGenerator.homePage, (Route<dynamic> route) => false);
  ...
  dynamic resetPassword(
    ...
    try {
      var result = await globalCollectionServices.resetPassword(context: context, password: password);
        ...
        if (res[0]['status'] == 'Gagal') {
          ...
          if (isBuat) {
            await DialogUtils.instance.showInfo(
              context: context,
              title: "Pemberitahuan!",
              text: "Password berhasil dibuat, Anda akan segera dialihkan ke halaman PIN",
              isCancel: false,
              clickOKText: "OK",
            );
          } else {
  ...
  dynamic resetPin(
    ...
    if (isBuat) EasyLoading.show(status: config.Loading);
    try {
      var result = await globalCollectionServices.resetPin(context: context, pin: pin);
      var res = result == null ? null : json.decode(result);
      EasyLoading.dismiss();
      if (res == null) {
        DialogUtils.instance.showError(context: context);
        ...
        if (res[0]['status'] == 'Gagal') {
          ...
          if (isBuat) {
            await DialogUtils.instance.showInfo(
              context: context,
              title: "Pemberitahuan!",
              text: "PIN berhasil dibuat, Anda akan segera dialihkan ke halaman Login",
              isCancel: false,
              clickOKText: "OK",
            );
          } else {
  ...
  Future<dynamic> uploadProgressLocalNotification(
    ...
    await flutterLocalNotificationsPlugin.show(123, "Upload transaksi offline", progress.toString() + " of " + maxProgress.toString() + " Uploaded", platformChannelSpecifics);
  }
  ...
  dynamic exportDB(BuildContext context, String path) async {
    try {
      ...
      if ((await Permission.manageExternalStorage.request().isGranted || await Permission.storage.request().isGranted) && await Permission.accessMediaLocation.request().isGranted) {
        File a = await b.copy(newPath);
        DialogUtils.instance.showInfo(context: context, title: 'Database Tersimpan', text: "Database berhasil disimpan pada $a", isCancel: false, clickOKText: "TUTUP");
      } else {
  ...
  dynamic checkUpdate(BuildContext context) async {
    try {
      ...
      if (resp == null) {
        _updateInfo = new UpdateInfo(title: '', version: '', desc: '', type: 'NO_UPDATE');
      } else {
        _updateInfo = resp;
      }
    } catch (e) {
